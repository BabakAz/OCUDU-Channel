...
- Write human-facing prose -- commits, PRs, README, the technical reference, the agent files -- by the `agent-writing` rules (from `zhouyou-gu/skill-marketplace-template`): terse, concrete, evidence-bound. Cut filler openers ("It's worth noting", "Importantly"), hedge stacks, and marketing adjectives ("powerful", "seamless", "robust", "comprehensive"); name the file / function / number / command instead of "various improvements"; prefer active voice and grammatically parallel lists; expand an acronym on first use; and state known limits and caveats plainly rather than glossing them.
- One name per concept across a whole document; do not alternate synonyms for variety. Where the canonical prose term differs from a code or config identifier, use the prose term in prose and keep the literal token only where it names code/API (e.g. say "edge" in prose; keep `link_id` / `link_key` / `links:` as API names). Record the canonical-vs-API split once in the doc's glossary.
- State a measured number with one value and one rounding everywhere it appears -- the README, the doc hero, and the technical-reference body must not disagree (not "≈180×" in one place and "183×" in another). This is the consistency complement to the measured-envelopes rule above.
- After any doc restructure or section renumbering, re-verify before reporting the turn complete that every internal anchor and every README→doc section-number citation still resolves; renumbering silently breaks "see §N" prose and cross-file links.
